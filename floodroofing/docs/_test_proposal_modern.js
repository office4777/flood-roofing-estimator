// Modernised proposal: end-to-end visual sample.
//
// Seeds S with a realistic 4-face hip roof, two quote options, a Maxam
// colour palette, gallery + job photos, and slot assignments — then
// captures the full proposal preview at high DPI.
const { chromium } = require('playwright');
const path = require('path');

(async () => {
  const browser = await chromium.launch({ executablePath: '/opt/pw-browsers/chromium-1194/chrome-linux/chrome' });
  const ctx = await browser.newContext({ deviceScaleFactor: 2, viewport: { width: 1320, height: 1500 } });
  ...
  await page.evaluate(() => {
    ...
    // ── Solid-colour SVG tile so we don't need any network for photos ──
    ...
    // ── Seed a realistic 4-face hip roof in S.roofData ──
    // Outline: rectangle 10..90 horizontally, 20..80 vertically (pct).
    // 4 hip faces meeting at a central horizontal ridge from (30,50) to (70,50).
    window.S.roofData = {
      roof_type: 'Hip',
      area_m2: 148.6,
      pitch: 22,
    ...
    // ── Land on the Quote tab and re-render ──
  ...
  // Find the proposal preview height + screenshot full
  const proposal = await page.$('#quoteProposal');
  if (proposal){
    const box = await proposal.boundingBox();
    if (box && box.height > 0) {
      await page.setViewportSize({ width: 1320, height: Math.min(8500, Math.ceil(box.height + 60)) });
      await page.waitForTimeout(200);
    }
    await proposal.screenshot({ path: 'floodroofing/docs/proposal_modern_full.png' });
    console.log('wrote floodroofing/docs/proposal_modern_full.png  ('+Math.round(box.height)+'px tall)');
  }

  // Per-page screenshots for the test proposal so we can review individually.
  const pages = await page.$$('#quoteProposal .rp-page, #quoteProposal .rp-header');
  for (let i = 0; i < pages.length; i++){
    await pages[i].screenshot({ path: 'floodroofing/docs/proposal_modern_page_'+(i+1)+'.png' });
  }
  console.log('wrote '+pages.length+' per-page screenshots');
