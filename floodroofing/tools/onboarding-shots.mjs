...
// step shows the aerial of the property.
{
  ...
  await step('move', 'Move / Edit is pointed at first — the tool for dragging the picture.');
  ...
  await pg.evaluate(() => document.getElementById('tourNext').click());
  await step('rotate-open', 'The rotate control lives under this button; it is pointed at before the slider is.');
  await pg.click('#viewMenuBtn');
  await step('rotate', 'The slider itself, ringed. Optional — Skip.');
  await pg.evaluate(() => document.getElementById('tourNext').click());
  ...
  await step('checklist', 'The checklist, pre-ticked on the practice job, with a Tick-every-line box for real jobs. Shown for three seconds.');
  ...
  await step('supplier', 'The supplier list, for three seconds: set up your default suppliers in Settings.');
