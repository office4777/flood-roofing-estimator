...
v = await pg.evaluate(() => ({
  ...
  order: Array.from(document.querySelectorAll('.sched-row:not(.pad):not(.sched-fold):not(.sched-add):not(.sched-gap) .sched-cell:nth-child(2)')).map(e => e.textContent.trim()),
}));
...
check('collapsing a folder hides its jobs but keeps its heading', await (async () => {
  ...
  return pg.evaluate(() => {
    ...
    const names = Array.from(document.querySelectorAll('.sched-row:not(.pad):not(.sched-fold):not(.sched-add):not(.sched-gap) .sched-cell:nth-child(2)')).map(e => e.textContent.trim());
