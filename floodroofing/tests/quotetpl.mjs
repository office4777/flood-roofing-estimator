...
check('THE ASK: the Quote tab then offers the saved quotes to pick from',
  // The saved templates live in the Select from saved templates menu since 2026-09-22.
  (await pg.evaluate(() => [...document.querySelectorAll('#qaTplList button')].map(o => o.textContent).join('|')))
...
// ── the quote action bar ──────────────────────────────────────────
{
  const bar = await pg.evaluate(() => ({
    ...
    picker: (document.querySelector('#qaTplMenu > summary') || {}).textContent || '',
  }));
